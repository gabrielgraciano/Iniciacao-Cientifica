...
##fazer boxplot (é possível fazer um boxplot?)
```{r}
dados%>%
...
Sim. Ambos os grupos apresentaram contagens menores de linfocitos TCD4 se comparados ao inicido do seguimento.

7) Construa um gráfico para visualizar a distribuição da carga viral no inicio e no final do seguimento no grupo de crianças que usaram HAART.
...
g1 <-
  ...
  scale_y_continuous(limits = c(300, 340))+
  geom_boxplot()
g2 <-
  dados%>%
  filter(haart == 'Sim')%>%
...
11) Com base nos objetivos, faça um resumo dos resultados.
O objetivo era comparar dois grupos de crianças: haart + e haart -. Observou-se que o medicamento é eficiente na amenização das consequências do HIV. Um único ponto que me chamou a atenção foi a divisão por sexo: enquanto feminino utilizou haart, masculino não.
